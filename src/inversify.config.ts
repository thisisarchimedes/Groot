...
export class InversifyConfig {
  ...
  private bindContracts() {
    this.container.bind<PositionLedgerContract>(TYPES.PositionLedgerContract)
        .toDynamicValue(async (context: interfaces.Context) => {
          const config = context.container.get<IConfigService>(TYPES.IConfigServiceAWS);
          const abiRepo = context.container.get<IAbiRepo>(TYPES.IAbiRepo);

          const positionLedgerAddress = config.getLeverageContractInfo().positionLedger;

          const positionLedgerABI = await abiRepo.getAbiByAddress(positionLedgerAddress);

          return new PositionLedgerContract(positionLedgerAddress, positionLedgerABI);
        }).inSingletonScope();
  }
